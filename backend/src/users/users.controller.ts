...
export class UsersController {
  ...
  @ApiResponse({ status: 201, description: 'สร้างผู้ใช้สำเร็จ', type: RequestCreateUserDto })
  ...
  @Get()
  @ApiOkResponse({ description: 'ดึงข้อมูลผู้ใช้ทั้งหมด', type: [RequestCreateUserDto] }) // Swagger Response
  findAll() {
    return this.usersService.findAll()
  }

  @Get(':id')
  @ApiOkResponse({ description: 'ดึงข้อมูลผู้ใช้ด้วย ID', type: RequestCreateUserDto }) // Swagger Response
  ...
  @Patch(':id')
  @UsePipes(ValidationPipe)
  @ApiOkResponse({ description: 'แก้ไขข้อมูลผู้ใช้', type: RequestUpdateUserDto }) // Swagger Response
  update(@Param('id') id: string, @Body() RequestUpdateUserDto: RequestUpdateUserDto) {
